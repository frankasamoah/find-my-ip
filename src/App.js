    ...
    try {
      const getInitialData = async () => {
        const { data } = await axios(
          `https://geo.ipify.org/api/v2/country,city?apiKey=${process.env.REACT_APP_API_KEY}&ipAddress=195.138.42.15`
        );
        setAddress(data);
      };
  ...
  const getTypedData = async () => {
    const { data } = await axios(
      `https://geo.ipify.org/api/v2/country,city?apiKey=${
        ...
        veryIpAddress.test(ipAddress)
          ? `ipAddress=${ipAddress}`
          : veryDomain.test(ipAddress)
          ? `domain=${ipAddress}`
          : ""
      }`
    );
        ...
        <div className="max-w-xl mx-auto p-8">
          ...
          >
            <input
              ...
              name="ipaddress"
              id="ipaddress"
              placeholder="Type any ip address or domain"
